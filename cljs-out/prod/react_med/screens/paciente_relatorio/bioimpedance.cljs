...
(defn massa-livre-de-gordura
  ...
  (let [idade (-> (js/Date. data)
                ...
                (js/Math.floor))]
    (cond
      (<= 8 idade 14) (eq2-male-and-female-children-from-8-15-years avaliacao-info)
      (= atividade-fisica "Atleta") (eq12-male-and-female-athletes avaliacao-info) ;; ou eq1?
      (and (= sexo "Feminino")
           (<= 15 idade 34)
           (= atividade-fisica "Ativo")) (eq4-females-active-18-35-years avaliacao-info)
      (and (= sexo "Feminino")
           (<= 15 idade 29)) (eq3-females-18-30-years avaliacao-info)
      (and (= sexo "Feminino")
           (<= 30 idade 49)) (eq5-females-30-50-years avaliacao-info)
      (and (= sexo "Feminino")
           (<= 50 idade 69)) (eq6-females-50-70-years avaliacao-info)
      (and (= sexo "Masculino")
           (<= 15 idade 29)) (eq7-males-18-30-years avaliacao-info)
      (and (= sexo "Masculino")
           (<= 30 idade 49)) (eq8-males-30-50-years avaliacao-info)
      (and (= sexo "Masculino")
           (<= 50 idade 69)) (eq9-males-50-70-years avaliacao-info)
      (= sexo "Masculino") (eq10-men avaliacao-info)
      (= sexo "Feminino") (eq11-women avaliacao-info))))
...
(defn agua-corporal
  [{:keys [data nascimento sexo atividade-fisica] :as avaliacao-info}]
  {:pre [(util/validate
           ::avaliacao-info
           avaliacao-info
           "Informações de avaliação inválidas.")]
  ...
  (let [idade (-> (js/Date. data)
                  ...
                  (js/Math.floor))]
    (cond
      (= atividade-fisica "Atleta") (eq13-tbw-male-and-female-athletes avaliacao-info)
      (= sexo "Masculino") (eq16-tbw-men avaliacao-info)
      (= sexo "Feminino") (eq17-tbw-women avaliacao-info))))
...
;; eq13-eq20 ou eq16-eq18 Ou eq16-eq20?
(defn eq19
  "TBW-ECW"
  [avaliacao-info]
  (- (eq13-tbw-male-and-female-athletes avaliacao-info)
     (eq18-ecw-male-and-female avaliacao-info)))

;; eq13-eq20 ou eq17-eq18 Ou eq16-eq20?
(defn eq21
  "TBW-ECW"
  [avaliacao-info]
  (- (eq13-tbw-male-and-female-athletes avaliacao-info)
...
(defn impedancia
  ...
  {:pre [(util/validate ::avaliacao-info avaliacao-info
           "Informações de avaliação inválidas.")]
   :post [(util/validate pos? % "Função impedancia não retornou um número positivo.")]}
...
(defn t [avaliacao-info]
  ...
  (js/console.log "agua + musculoesqueletico + gordura, relativo ao peso"
                  (/ (+ (agua-corporal avaliacao-info)
                        (massa-musculoesqueletica avaliacao-info)
                        (- (:peso avaliacao-info) (massa-livre-de-gordura avaliacao-info)))
                     (:peso avaliacao-info)))
  )
